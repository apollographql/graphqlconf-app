...
internal fun getStringBlocking(resource: StringResource, vararg formatArgs: Any): String {
  return runBlocking { getString(resource, *formatArgs) }
}

fun Context.configureNotifications() {
  val name = getStringBlocking(Res.string.channel_name)
  val descriptionText = getStringBlocking(Res.string.channel_description)
...
class NotifyWorker(
  ...
  override suspend fun doWork(): Result {
    ...
    with(context) {
      ...
      // Android resources declared in a KMP library's androidMain can't be referenced via R
      // from this source set, so look up the drawable merged into the final APK at runtime.
      val smallIconResId = resources.getIdentifier("graphql", "drawable", packageName)
      val notification = NotificationCompat.Builder(this, notificationChannelId)
        .setSmallIcon(smallIconResId)
        .setContentTitle(getStringBlocking(Res.string.notification_title, title))
        .setContentText(getStringBlocking(Res.string.notification_description, room))
