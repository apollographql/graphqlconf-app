package apollo.compose.compiler.ir

import apollo.compose.compiler.fir.ApolloFirDeclarationGenerationExtension
...
class SimpleIrBodyGenerator(pluginContext: IrPluginContext) : AbstractTransformerForGenerator(pluginContext) {
    override fun interestedIn(key: GeneratedDeclarationKey?): Boolean {
        return key == ApolloFirDeclarationGenerationExtension.Key
    }

    override fun generateBodyForFunction(function: IrSimpleFunction, key: GeneratedDeclarationKey?): IrBody {
        require(function.name == ApolloFirDeclarationGenerationExtension.FOO_ID.callableName)
